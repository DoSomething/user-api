...
function togglePreregistrationPrompt(event) {
  ...
  const preRegisteredContent = document.getElementById(
    'voter-reg-cta-pre-registration',
  );
...
function clickHandlerToggleContent(event) {
  ...
  const unregisteredContent = document.getElementById(
    'voter-reg-cta-unregistered',
  );
  const preRegisteredContent = document.getElementById(
    'voter-reg-cta-pre-registration',
  );
  ...
  if (
    value === 'unregistered' &&
    preRegisteredContent.classList.contains('hidden')
  ) {
...
const init = () => {
  $(document).ready(() => {
    const voterRegStatusInputs = document.getElementsByClassName(
      'voter-reg-status',
    );
